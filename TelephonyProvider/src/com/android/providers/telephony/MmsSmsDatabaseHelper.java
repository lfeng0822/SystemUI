...
//            + Mms.THREAD_ID + "; " +
    ...
    static final int DATABASE_VERSION = 601020;
...
//                   "END;");
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int currentVersion) {
        ...
        case 601010:
            ...
            db.beginTransaction();
            try {
                upgradeDatabaseToVersion601020(db);
                db.setTransactionSuccessful();
            } catch (Throwable ex) {
                Log.e(TAG, ex.getMessage(), ex);
                break;
...
//        createRCSTable(db);
    ...
    private void upgradeDatabaseToVersion601020(SQLiteDatabase db) {
        db.execSQL("DROP TRIGGER IF EXISTS update_threads_error_on_move_mms");
        db.execSQL("CREATE TRIGGER update_threads_error_on_move_mms " +
                   "  BEFORE UPDATE OF msg_box ON pdu " +
                   "  WHEN ((OLD.msg_box = 4 OR OLD.msg_box = 5) " +
    ...
    private void createRCSTriggers(SQLiteDatabase db) {
        ...
        db.execSQL(
            ...
            "    SET " +
            "      " + RCSProvider.CONVERSATION_COLUMN_BODY +
                        "=NEW." + RCSProvider.MESSAGE_COLUMN_BODY + ", " +
            "      " + RCSProvider.CONVERSATION_COLUMN_TIMESTAMP +
                        "=NEW." + RCSProvider.MESSAGE_COLUMN_TIMESTAMP + ", " +
            ...
            "      " + RCSProvider.CONVERSATION_COLUMN_MIME_TYPE +
                        "=NEW." + RCSProvider.MESSAGE_COLUMN_MIME_TYPE + ", " +
            "      " + RCSProvider.CONVERSATION_COLUMN_TYPE +
                        "=NEW." + RCSProvider.MESSAGE_COLUMN_TYPE + ", " +
            "      " + RCSProvider.CONVERSATION_COLUMN_ATTACHMENT + "=(CASE " +
            "               WHEN NEW." + RCSProvider.MESSAGE_COLUMN_TYPE + " = " +
            "      " +        RCSProvider.TYPE_FT + " THEN 1 " +
            "               ELSE " + RCSProvider.CONVERSATION_COLUMN_ATTACHMENT + " END), " +
            "      " + RCSProvider.CONVERSATION_COLUMN_READ + "=(CASE " +
            "               WHEN NEW." + RCSProvider.MESSAGE_COLUMN_MESSAGE_STATUS + "=" +
            "      " +        RCSProvider.MESSAGE_STATUS_UNREAD + " THEN 0 " +
            "               ELSE " + RCSProvider.CONVERSATION_COLUMN_READ + " END), " +
            "      " + RCSProvider.CONVERSATION_COLUMN_CLASS +
                        "=NEW." + RCSProvider.MESSAGE_COLUMN_CLASS + " " +
        ...
        db.execSQL(
            ...
            "                   CASE WHEN NEW." + RCSProvider.MESSAGE_COLUMN_MESSAGE_STATUS + "=" +
                                    RCSProvider.MESSAGE_STATUS_FAILED + " THEN 1 " +
            "                   ELSE " + RCSProvider.CONVERSATION_COLUMN_ERROR + " END) " +
            "    WHERE " + RCSProvider.TABLE_CONVERSATIONS + "." +
                            RCSProvider.CONVERSATION_COLUMN_CONVERSATION + "=NEW." +
                            RCSProvider.MESSAGE_COLUMN_CONVERSATION + "; " +
            " END;");
